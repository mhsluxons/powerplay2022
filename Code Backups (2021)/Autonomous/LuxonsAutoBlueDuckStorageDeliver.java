...
//FACES FORWARDS, THEN BACKWARDS
@Autonomous(name = "RB Duck Storage Deliver") //parks in the blue storage area and delivers block
//@Disabled
public class LuxonsAutoBlueDuckStorageDeliver extends LinearOpMode {
    ...
    //RPM for the duck motor (needs checking)
    public double DUCK_MOTOR_RPM = 30.0;
    ...
    public void runOpMode() {
        ...
        //move robot left towards duck carousel
        shiftRobot("right", 8);
        idle();
        sleep(2000);
        finishRun();
        ...
        //rotates the duck carousel
        duck_motor.setPower(DUCK_MOTOR_RPM);
        sleep(2000);

        //turns off duck motor
        duck_motor.setPower(0);
        sleep(1500);

        //moves forward to the storage parking area
        run("front", 16);
        idle();
        sleep(2500);
        finishRun();
        ...
        //rotate the robot to face backwards
        spin("right", 12); //turns right to face the back of the robot towards the storage unit
        idle();
        sleep(2000);
        finishRun();
        idle();

        //moves the robot closer to the wall
        shiftRobot("left", 3);
        idle();
        sleep(1000);
        finishRun();
        ...
        //delivers block
        block_motor.setPosition(BLOCK_END);
        sleep(2000);

        //moves robot and block into storage unit
        shiftRobot("back", 8);
        idle();
        sleep(2000);
        finishRun();
        ...
        //sets deliverer back to normal
        block_motor.setPosition(BLOCK_INITIAL);
        sleep(2000);
...
//
        int frontRightTicks = ticks*45/100;
//        int backRightTicks = ticks*80/100;
        ...
        if (type.equalsIgnoreCase("front")) {
            ...
            rightFrontMotor.setTargetPosition(rightFrontMotor.getCurrentPosition() + frontRightTicks);
...
//        double backRPowertemp = powerForward[0] *65/100;
        ...
        rightFrontMotor.setPower(frontRPowertemp);
