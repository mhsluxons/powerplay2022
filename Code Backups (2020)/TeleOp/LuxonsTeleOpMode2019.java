...
//        }
    ...
    public void robotMove() {
        ...
        /*else if (turnLeft == 0.5) {
            frontRPower = turnValue * 0.5;
            backLPower = turnValue * 0.5;
            frontLPower = -turnValue * 0.5;
            backRPower = -turnValue * 0.5;
        }*/
    ...
    public void grabShoot() { //intake
        double drive = 1.0;
        if (gamepad2.left_trigger > 0) { //pulls rings in
            robot.grab_motor.setPower(drive);
        } else {
            robot.grab_motor.setPower(0);
        }
        if (gamepad2.left_stick_x > 0) { //pushes rings out (in case of emergency)
            robot.grab_motor.setPower(-drive);
        } else {
            robot.grab_motor.setPower(0);
        }
        ...
        if (gamepad2.right_trigger > 0) {
            robot.shoot_motor.setPower(0.90); //high power outtake (for the high goals)
        }
        else if (gamepad2.dpad_left == true) {
            robot.shoot_motor.setPower(0.85); //mid-high power outtake
        }
        else if (gamepad2.right_bumper == true) {
            robot.shoot_motor.setPower(0.80); //medium power outtake
        }
        else if (gamepad2.dpad_right == true) {
            robot.shoot_motor.setPower(0.75); //mid-low power outtake (for the power shots)
        }
        else if (gamepad2.left_bumper == true) {
            robot.shoot_motor.setPower(0.70); //low power outtake
        }
        else {
        ...
        /*if (gamepad2.right_stick_y == -1) {
            ...
            robot.ringPush.setPower(-0.4); //kicks ring into outtake flywheel
        }
        else {
            robot.ringPush.setPower(0); //retracts servo kicker
        }

        if (gamepad2.dpad_up == true) { //moves arm towards wobble goal
            move("front", 1);
        }
        if (gamepad2.dpad_down == true) { //moves arm away from wobble goal
            move("back", 1);
        }
        //if (gamepad2.dpad_left == true) {

        //}
        if (gamepad2.x == true) { //lets go of wobble goal
            robot.wobbleGrab.setPower(0.5);
        }
        if (gamepad2.b == true) { //grabs wobble goal
            robot.wobbleGrab.setPower(-0.6);
        }
        if (gamepad2.y == true) { //drops wobble goal over the wall
            move("front", 2);
            sleep(500);
            robot.wobbleGrab.setPower(0.5);
            sleep(400);
        ...
        if (gamepad2.a == true) { //sets wobble goal grabber to grab
            move("front", 6.5);
            robot.wobbleGrab.setPower(0.5);
        }

    }
    private void move(String type, double distance) {

        final double TicksperRev = 1500.0;
        int ticks = (int)((distance * 120.0 / 360.0) / 4.0 * TicksperRev);

        if (type.equalsIgnoreCase("front")) {
            robot.wobble_motor.setTargetPosition(robot.wobble_motor.getCurrentPosition() - ticks);
        }
        ...
        robot.wobble_motor.setMode(DcMotor.RunMode.RUN_TO_POSITION);

        double powerForward[] = {0.5, 0.8};

        robot.wobble_motor.setPower(powerForward[0]);
